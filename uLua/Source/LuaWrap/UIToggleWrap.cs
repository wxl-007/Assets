...
public class UIToggleWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("validator", get_validator, set_validator),
	...
	static int get_validator(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name validator");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index validator on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.validator);
		return 1;
	}
	...
	static int set_validator(IntPtr L)
	{
		...
		{
			obj.validator = (UIToggle.Validate)LuaScriptMgr.GetNetObject(L, 3, typeof(UIToggle.Validate));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			obj.validator = (param0) =>
			{
