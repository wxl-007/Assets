...
--初始化变量
	...
	self.cardsBg = self.transform:FindChild("Output/blackBg");
	self.cardsBg.gameObject:SetActive(false)
	...
	if self.gameObject.name == "User" then
		...
		self.userNickname = nil;
		self.userIntomoney = nil;
		self.m_gold_type = self.transform:FindChild("Output/CardType_gold").gameObject;
...
function this:Awake()
	...
	if self.gameObject.name ~= "User" then
        ...
        GameSRFTWZ.mono:AddClick(btn_Avatar,self.OnClickInfoDetail,self);
	else
		--看底牌按钮
		local card = self.cardsArray[1].gameObject;
		GameSRFTWZ.mono:AddClick(card, self.firstCardMOver,self);
	end
...
function this:Start()
	...
	if anchor.side == UIAnchor.Side.Right then
		...
		self.cardTypeTrans.localPosition = Vector3.New(-self.cardTypeP.x-10, self.cardTypeP.y, self.cardTypeP.z);
		self.showObj.transform.localPosition = Vector3.New(-self.showP.x-10, self.showP.y, self.showP.z);
	...
	elseif anchor.side == UIAnchor.Side.Left then
		self.parentX = 200;
		self.cardsTrans.localPosition = Vector3.New(self.parentX, self.parentY, self.parentZ);
		local tShowObjPos = self.showObj.transform.localPosition
		self.showObj.transform.localPosition =  Vector3.New(203, tShowObjPos.y, tShowObjPos.z);
		self.cardTypeTrans.localPosition = Vector3.New(self.cardTypeP.x+10, self.cardTypeP.y, self.cardTypeP.z);
	end
	self.allObj.transform.localPosition = self.showObj.transform.localPosition
	self.guoObj.transform.localPosition = self.showObj.transform.localPosition
	self.qibj.transform.localPosition = self.showObj.transform.localPosition
	self.cardsBg.gameObject.transform.localPosition = Vector3.New(self.parentX, self.parentY, self.parentZ);
	self._alive = true;
end
...
function this:SetSelfH1_2(  firstCardNum, secCardNum,startPos)
	self.cardsTrans.gameObject:SetActive(true);
	...
	self.cardsBg.gameObject:SetActive(true)
	self.firstcNum = firstCardNum;
end

function this:SetSelfH3_4_5( ind, secCardNum,delay)
	self.cardsTrans.localPosition = Vector3.New(self.parentX+15*(5-ind)-15, self.parentY, self.parentZ);
	if ind==3 then
		self.cardsTrans.localPosition = self.cardsTrans.localPosition + Vector3.New(5,0,0)
	end
	local tDelayTime = 1
	if delay ~= nil then
		tDelayTime = delay
...
function this:SetOtherH3_4_5( ind, secCardNum,delay)
	...
	self.cardsArray [1].transform.localPosition = self.firstCardPosition;
	self.cardsArray [2].transform.localPosition =  Vector3.New(self.firstCardPosition.x+35,self.firstCardPosition.y,0);
	self:faPai(self.cardsArray[ind], tostring(secCardNum),self.cardsArray [ind].transform.localPosition.x,self.parentY-200,0.5,tDelayTime);
end
...
function this:clearPais()
	...
	local tGoldLabelTransGroup = self.transform:FindChild("Output/CardType_goldLabelGroup")
	if tGoldLabelTransGroup~=nil then
		tGoldLabelTransGroup.gameObject:SetActive(false)
	end
end
...
---主玩家的比牌情况
function this:SetCardTypeUser(cardType,isGold,extGoldCount)
	self.cardsArray [1].spriteName = self._cardPre..self:transformCardInfo(self.firstcNum) ;
	self.firstcNum = -1;
	...
	local tGoldLabelTransGroup = self.transform:FindChild("Output/CardType_goldLabelGroup")
	if tGoldLabelTransGroup~=nil then
		if isGold == nil or  tostring(isGold)==0 or cardType==-1 then
			self.m_gold_type:SetActive(false)
			tGoldLabelTransGroup.gameObject:SetActive(false)
        elseif tostring(isGold) == "1" then
			self.m_gold_type:SetActive(true)
			tGoldLabelTransGroup.gameObject:SetActive(true)
			local tGoldLabelTransForm = self.transform:FindChild("Output/CardType_goldLabelGroup/Label/Label")
			if tGoldLabelTransForm~=nil  then
				local tGoldLabel = tGoldLabelTransForm.gameObject:GetComponent("UILabel");
				if tostring(cardType)=="7" then
					tGoldLabel.text = "恭喜你中了四梅"
				elseif  tostring(cardType)=="8" then
					tGoldLabel.text = "恭喜你中了同花顺"
				else
					tGoldLabel.text = "恭喜你"
				end
			end
			local tGoldLabelTransForm1 = self.transform:FindChild("Output/CardType_goldLabelGroup/Label")
			if tGoldLabelTransForm1~=nil  then
				local tGoldLabel1 = tGoldLabelTransForm1.gameObject:GetComponent("UILabel");
				tGoldLabel1.text = "大奖"..tostring(extGoldCount).."游戏币到银行了"
			end
			if self.m_gold_type.activeSelf then
				error("visible")
			else
				error("novisible")
			end
...
function this:SetCardTypeOther( firCardNum, cardType,isGold)
	self.allObj:SetActive (false);
	self.showObj:SetActive (false);
	self.guoObj:SetActive (false);
	...
	if isGold == nil then
		self.m_gold_type:SetActive(false)
	elseif isGold == 1 or isGold=="1" then
		self.m_gold_type:SetActive(true)
	end
end
...
function this:SetStartChipOther2( parent,  jetton, parent1)
	...
	coroutine.start(self.AfterDoing,self,0.5,function() parent1.gameObject:SetActive(false) end);
end
...
function this:SetStartChipOther( parent,jetton)
	...
	if parent.name == "leftScore1" then
		table1 = iTween.Hash("position",Vector3.New(vexs.x-100, vexs.y, 0),"time", 0.5,"islocal", true);
	elseif  parent.name == "rightScore1" then
		table1 = iTween.Hash("position",Vector3.New(vexs.x+100, vexs.y, 0),"time", 0.5,"islocal", true);
	end
...
function this:faPai( gObj, pai, x, y, _time, delay)
	local tempRun = function ()
		...
		local tGameObj = GameObject.Find("GameSRFTWZ")
		if tGameObj then
