...
function this:Init()
--初始化变量
	self.TimerItem1 = self.transform:FindChild("SpriteL").gameObject:GetComponent("UISprite");
	self.TimerItem2 =self.transform:FindChild("SpriteR").gameObject:GetComponent("UISprite");
	--log(self.TimerItem1.name);
	--log(self.TimerItem2.name);
	self.m_prefix = ""
	self.isStart = false;
	self.endTime = 0;
	self.m_run = nil
...
function this:Update()
	if self.isStart then
		if self.endTime > 0 then
			...
			coroutine.start(self.ShowCurTimer,self,self.endTime);
		else
			self.isStart = false;
			self:OnTimeComplete()
		end
...
function this:ClearTimer()
	--self:ShowCurTimer (0);
	coroutine.start(self.ShowCurTimer,self,0);
end
--截取字符串分别显示倒计时的时间
function this:ShowCurTimer( _time)
	...
	--log("时间大于10");
		...
		self.TimerItem1.gameObject:SetActive(true);
		self.TimerItem2.transform.localPosition=Vector3.New(-194,9,0);
	...
	--log("时间小于10");
		...
		self.TimerItem1.gameObject:SetActive(false);
		self.TimerItem2.transform.localPosition=Vector3.New(-212,9,0);
		self.TimerItem1.spriteName = self.m_prefix..Define.NN_TIMER_NUM[t1+1];
		self.TimerItem2.spriteName = self.m_prefix..Define.NN_TIMER_NUM[t2+1];
		if t2==1 and GamePPC.NeedEnabled then
		    --log("倒计时结束隐藏额度和续压");
			coroutine.wait(0.5);
			GamePPC:NotClick();
		end
