...
function  this.init()
	...
	self._cardPre = "card_";
	self.ownCard_prb=ResManager:LoadAsset("gamehpsk/prefab","ownCard");--实例化自己的牌的预制件
	self.selectedCards={};
	self.OtherOutCard=-1;
	self.OtherOutCardType=-1;
	self.paixing=-1;
...
function this.PutUICards(count,cardsList,isBanker)
	...
	for i = 1, #self.library do 	
		...
		if i==count then
			--手里牌排序
			self.CardObjectList=HPSKCardRule.SortCardsFunc(self.CardObjectList,true)
			...
			if isBanker then
				...
				GameHPSK:SetTiShi(true,0);
				GameHPSK.UserPlayerCtrl:HideOrShowTime(true);
			end
		end
...
function this.Update()
	--log("-------++++++++++++--------Update---------+++++++++-------");
	if GameHPSK.biaoqingBgShow==true then
		return;
	end
	if GameHPSK.tuoguanBgShow==true then
		return;
	end
...
function this.IsCanOutCard(cardtype,cardArray,cardCount)
	...
	if isChu then
		GameHPSK:SetTiShi(false,cardtype);
	else
		GameHPSK:SetTiShi(false,0);
	end
end
...
function this.OutCard()
	...
	GameHPSK.autoOutCardList=cards;
	
	local outcard = {type="shuangkou",tag="play",body=cards}; 	
	local jsonStr = cjson.encode(outcard);
	GameHPSK.mono:SendPackage(jsonStr);
	
	
	local cardParent=transform:FindChild("cardParent");
	for i=1,#(self.selectedCards) do
		self.selectedCards[i].GameObject.transform.parent=GameHPSK.outCardParent;
		self.selectedCards[i].GameObject.transform.localScale=Vector3.one;
		self.selectedCards[i].GameObject.transform.localPosition=Vector3.New(50*(i-1),0,0);
		self.selectedCards[i].GameObject.transform:GetComponent("UISprite").depth = 10+ i
	end
	GameHPSK.outCardParent.transform.localPosition=Vector3.New(-25*(#(cards)-1),GameHPSK.outCardParent.localPosition.y,0);
	for i=1,#(self.selectedCards) do
	...
	if #(self.CardObjectList)>0 and self.paixing>10 then
		GameHPSK:PlayCardSound(self.paixing,GameHPSK.UserPlayerCtrl.sex);
	end
...
function this.SetLatePutOut(cardtype,outcards)
	...
	for i=1,#(outcards) do
		local obj=GameObject.Instantiate(self.ownCard_prb);
		obj.transform.parent=GameHPSK.outCardParent;
		obj.transform.localScale=Vector3.one;
		obj.transform.localPosition=Vector3.New(50*(i-1),0,0);
...
function this.DestroyCards()
	local sprites = GameHPSK.outCardParent:GetComponentsInChildren(Type.GetType("UISprite",true));
