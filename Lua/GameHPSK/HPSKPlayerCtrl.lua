...
--初始化变量
	...
	self.otherCard_prb=ResManager:LoadAsset("gamehpsk/prefab","otherCard");--实例化别人的牌的预制件
	...
	if self.gameObject.name == "User" then
		...
		self.cardOutParent=self.transform:FindChild("Output/outCardParent");--出牌父物体
...
function this:Awake()
	...
	if self.gameObject.name ~= "User" then
		...
		GameHPSK.mono:AddClick(btn_Avatar,self.OnClickInfoDetail,self);
...
function this:SetDeal(toShow , count, isBanker, infos)
	...
	if not toShow then	
		...
		self.cardsTrans.gameObject:SetActive(true);
		if self._cardData and #(self._cardData)>0 then--自己的发牌动画
			if sex==0 then
				UISoundManager.Instance.PlaySound("man_begin");
			else
				UISoundManager.Instance.PlaySound("woman_begin");
			end
			HPSKPlayer.PutUICards(count,self._cardData,isBanker);
			--[[
			for i=1,count do
			...
			for i=1,tonumber(count) do--别人的发牌动画
				...
				coroutine.wait(0.2);
				if i==count  then
					GameHPSK.ButtonTuoguan:GetComponent("BoxCollider").enabled=true;
					if isBanker then
						coroutine.wait(0.2);
...
function this:SetOtherOutPai(Cards,cardType,num)
	...
	for i=1,#(outcards) do
		...
		outcards[i].transform.localPosition=Vector3.New(50*(i-1),0,0);
		outcards[i]:GetComponent("UISprite").depth=15+i;
	end
	self.cardOutParent.transform.localPosition=Vector3.New(-25*(#(outcards)-1),self.cardOutParent.transform.localPosition.y,0);
	...
	if cardType>10 then
		GameHPSK:PlayCardSound(cardType,self.sex);
	end
end
...
function this:SetLatePutOut(cardtype,cards)
	...
	for i=1,#(cards) do
		...
		handcard.transform.localPosition=Vector3.New(50*(i-1),0,0);
		handcard:GetComponent("UISprite").depth=15+i;
