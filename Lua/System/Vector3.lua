...
function Vector3:Nearby(vb,scope)
	if scope ==nil then  scope = 10; end
	
	if self.x+scope > vb.x and self.x-scope < vb.x  and self.y+scope > vb.y and self.y-scope < vb.y  and self.z+scope > vb.z and self.z-scope < vb.z then 
		return true;
	end  
	return false;
end
