...
function CJFRacePanel.Awake(obj)
	print("Awke CJFRacePanel");
	obj:SetActive(true)
	gameObject = obj
...
function self.init()
	...
	mono:AddClick(self.btnrule,CJFRacePanel.OnRuleCallBack)
	mono:AddClick(self.rule.transform:FindChild("btn_close").gameObject,CJFRacePanel.OnRuleCallBack)
	...
	mono:AddClick(self.btnrank,CJFRacePanel.OnRankCallBack)
	mono:AddClick(self.rank.transform:FindChild("btnclose").gameObject,CJFRacePanel.OnRankCallBack)


	self.btnclose = transform:FindChild("btnExit").gameObject
	mono:AddClick(self.btnclose,CJFRacePanel.OnCloseCallBack)

	self.btnApply = transform:FindChild("btnapply").gameObject
	self.btngoon = transform:FindChild("btngoon").gameObject
	mono:AddClick(self.btnApply,CJFRacePanel.OnApplyCallBack)
	mono:AddClick(self.btngoon,CJFRacePanel.OnApplyCallBack)
	...
	transform:FindChild("Label_ave"):GetComponent("UILabel").text = tostring(ave_score)
	transform:FindChild("Label_round"):GetComponent("UILabel").text = tostring(round)
	transform:FindChild("Label_rank"):GetComponent("UILabel").text = tostring(rank)
	...
	self.geticon = transform:FindChild("geticon").gameObject
	self.geticon:SetActive(false)

	self.lb_hasAward = transform:FindChild("Label_canGet"):GetComponent("UILabel")
	...
	self.waittingtimer = Timer.New(CJFRacePanel.CountWaittingTime,1,-1,true)
	--物品显示
	self.itembtn = {}
	for i=1,7 do
		local btn = transform:FindChild("item/btnitem"..i).gameObject
	...
	if tag == "apply" or restseconds == -1 or restseconds > 0 then
		self.btngoon:SetActive(false)
		self.btnApply:SetActive(true)
	else
		self.btngoon:SetActive(true)
		self.btnApply:SetActive(false)
	end

	self.timer = Timer.New(CJFRacePanel.CountDown,1,-1,true)--比赛开始倒计时
	CJFRacePanel.SetTime(restseconds)
...
function self.OnItemCallBack(go)
	...
	self.itemName.text = JDMatchAward[index].name
	self.itemNum.text = JDMatchAward[index].value
	self.itemUsed.text = JDMatchAward[index].usedesc
	self.itemDesc.text = JDMatchAward[index].otherdesc
	self.itemIcon.spriteName = JDMatchAward[index].icon
...
function CJFRacePanel.OnRuleCallBack(go)
	self.rule:SetActive(go.name == self.btnrule.name)
end
function CJFRacePanel.OnRankCallBack( go )
	self.rank:SetActive(go == self.btnrank)
end
function CJFRacePanel.OnCloseCallBack()
...
function CJFRacePanel.OnApplyCallBack( go)
...
local waittime = 0
function CJFRacePanel.Waitting( isshow )
...
function CJFRacePanel.OnDestroy()
	gameObject = nil
	transform = nil
	mono = nil
	self.min = nil
...
function CJFRacePanel.SetTime(time)
	cTime = time
...
function CJFRacePanel.CountDown()
	if cTime > 0 then
...
function CJFRacePanel.GetAwards(_awards)
	if _awards ~= nil then
		awards = _awards
	else
		return awards
...
function CJFRacePanel.SetPanel(messageObj)
...
function CJFRacePanel.LoadInof()
	--登录
	--[[
	--if recSession == nil then
    ...
    coroutine.www(infowww)
   	...
   	if info["result"] ~= nil and info["result"] == "ok" then
   		...
   		if body["add_coin"] ~= nil and body["add_coin"] > 0 then
   			self.lb_hasAward.text = body["add_coin"].."元宝"
		...
		if body["item_id"]~= nil and body["item_id"] > 0 then
   		...
   		self.lb_hasAward.text = ""
   		self.btnAward:GetComponent("UIButton").isEnabled = false
   	end
end
function CJFRacePanel:HttpRequestWithSession(pUrl,pForm,session)
	local tCookie =nil 
	if session ~= nil then
		tCookie = session
	else
