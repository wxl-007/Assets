...
self.ctype = CharacterType.Computer
self.sex = 2
self.id = 000002
self.name = "玩家2"
self.level = 1
...
function Computer.GetLibrary()
	return self.library
end 
--获取手牌张数
function Computer.GetCardsCount()
	return #self.library
end

function Computer.GetCardFromIndex(index)
	return self.library[index]
end 
--出牌
function Computer.PopCard(card)
...
function Computer.ClearDealCard()
...
function Computer.Awake(obj)
	gameObject = obj
	transform = obj.transform
	self.init()
end 
function Computer.OnDisable()
	self.Clear()
	self.ClearDealCard()
end 
--注销
function Computer.OnDestroy()
	self.Clear()
	self.ClearDealCard()
end 
...
--2等待开局
--3等待加倍
function Computer.SetComputerState(state)
...
function Computer.ShowWarning(isopen)
	self.Warning:SetActive(isopen)
end 
function Computer.DelayPlayAudio(holdNum)--播放报警音效
	local audio_name = ""
	if holdNum == 2 then
		audio_name = "onlyTwo"
		CharacterComputer.WarnTwoAnimator()
	elseif holdNum == 1 then
		audio_name = "onlyOne"
		CharacterComputer.WarnOneAnimator()
	end
	if audio_name ~= "" then
...
function Computer.CreatCardsOnRigntHand(num)
	--全部一样的牌
...
function  Computer.GradLord(calltimes)
    --抢地主动画
   	local rand = math.random(1,2)
	if  rand == 1 then
		CharacterComputer.RobLandLordAnimator()
	...
	CountDownPanel.CancelCountDown(false)
   	...
   	if calltimes ~= 0 then
   		Computer.ShowNotice(GameText.GradLord)	--显示字
   	else
   		Computer.ShowNotice(GameText.CallLord)
   	end
...
--不叫地主
function Computer.DisCallLord()
	local rand = math.random(1,2)
	if  rand == 1 then
		CharacterComputer.NoDealAnimator()
	else
		CharacterComputer.NoDealAnimator1()
	end
	--OrderCtrl.GradLordTrun(true)
	Computer.ShowNotice(GameText.DisCallLord)
	--播放音效
	LRDDZ_SoundManager.OtherHumanSound(self.ctype,"noCall",self.sex == 1)
end
--不抢地主
function Computer.DisGradLandLord()
	local rand = math.random(1,2)
	if  rand == 1 then
		CharacterComputer.NoDealAnimator()
	else
	...
	--OrderCtrl.GradLordTrun(true)
	Computer.ShowNotice(GameText.DisGradLord)
	--播放音效
...
function Computer.Double()
	...
	CharacterComputer.RobLandLordAnimator1()
	Computer.ShowNotice(GameText.Double)
end
function Computer.NoDouble()
	LRDDZ_SoundManager.OtherHumanSound(self.ctype,"noDouble",self.sex == 1)
	CharacterComputer.NoDealAnimator1()
	Computer.ShowNotice(GameText.DisDouble)
end

function Computer.DisPlayCard(  )
	local rand = math.random(1,2)
		if  rand == 1 then
			CharacterComputer.NoDealAnimator()
		else
...
function Computer.PlayCardByServer(cards,holdNum)
	...
	if cardsType==CardsType.OnlyThree or cardsType==CardsType.ThreeAndOne or cardsType==CardsType.ThreeAndTwo or cardsType==CardsType.Straight or cardsType==CardsType.DoubleStraight or cardsType==CardsType.TripleStraight or cardsType==CardsType.TripleStraightAndSingle or cardsType==CardsType.TripleStraightAndDouble or cardsType==CardsType.FourAndSingle or cardsType==CardsType.FourAndDouble then
		if OrderCtrl.bigest ~= CharacterType.Computer then --判断大你的牌是否是对方出的
			clip = LRDDZ_SoundManager.OtherHumanSound(self.ctype,"bigger",self.sex == 1); --播放大你音效
			normal = false
		end
	end
	...
	CharacterComputer.Deal(cards,cardsType,func)   --播放出牌动作
		...
		GameCtrl.ShowGameText(CharacterType.Computer) --显示文字		
		--判断游戏是否结束
		coroutine.wait(0.8)--等待特效播放完再判断输赢
end
function Computer.ShowDealCard(cards)
	EginTools.ClearChildren(self.PlacePoint.transform)
...
function Computer.ShowNotice(str)
...
function Computer.ShowCardsNum(isactive)
	print(isactive);
	self.cardNum:SetActive(isactive) 
	
end 
function Computer.HideCardsNum()
	self.cardNum.transform:GetComponent("UILabel").text = "0"
	self.cardNum:SetActive(false) 
end
function Computer.HidenNotice()
	self.Notice:SetActive(false)
	self.ShowWarning(false)
end 
function Computer.SetCardsNum(num,foldnum)
...
--显示明牌的牌
function Computer.ShowOpenHandCards(cards,isanim)
...
function Computer.UpdateOpenHandCards()
	Computer.ShowOpenHandCards(self.library)
end
function Computer.PlayCardTypeName(cardstype)
...
--剩余牌
function Computer.ResidueCards(cards)
	for i = 1, #cards do 
		local obj = LRDDZ_ResourceManager.Instance:LoadAsset("smallpoker","smallpoker","openHand",false)
		...
		MyCommon.Set2DCard(obj.transform,cards[i].suits,cards[i].weight)
