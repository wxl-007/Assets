require "GameTBSZ/TBSZPlayerCtrl"
...
function this:Awake()
	...
	local footInfoPrb = ResManager:LoadAsset("gametbsz/footinfoprb3","FootInfoPrb3")
	...
	if PlatformGameDefine.playform.IsPool then
		...
		local JiangChi=GameObject.Find("JiangChiPrb(Clone)")
...
function this:GetTbwzPlayerCtrl(tbName,tbObj)
	...
	if tbwzTemp == nil then
		...
		if not IsNil(tbObj) then
			this._tbwzPlayerCtrl[tbName] = TBSZPlayerCtrl:New(tbObj);
...
function this.ProcessUpdateIntomoney(messageStr)
	...
	if not IsNil(info) then
		FootInfo:UpdateIntomoneyString(TBSZPlayerCtrl:NumberAddWan(intoMoney));
	end
end
