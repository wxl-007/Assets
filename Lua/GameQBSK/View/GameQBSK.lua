require "GameQBSK/QBSKPlayerCtrl"
require "GameQBSK/QBSKPlayer"
require "GameQBSK/QBSKCardRule"
...
local huiheEnd=false;
local autoOutCard="";
local systemOutCard="";

local hasCardsPlayerCount=0;--当前手里还有牌的玩家数
local notHasCardsPlayerCount=0;--手里没有牌的玩家个数
local shijiCount=0;

local passCardIndex=0;--出牌人之后过牌的总人数
...
function this:clearLuaValue()
	...
	huiheEnd=false;
	hasCardsPlayerCount=0;
	notHasCardsPlayerCount=0;
	shijiCount=0;
...
function this:Init()
	...
	this.outCardParent=this.transform:FindChild("Content/outCardParent/outCardParent");
	...
	--this.jiesuan_3=this.jiesuanMessage.transform:FindChild("jiesuan_3").gameObject;
	--this.jiesuan_4=this.jiesuanMessage.transform:FindChild("jiesuan_4").gameObject;
	this.jiesuan={};
	for i=1,4 do
		table.insert(this.jiesuan,this.jiesuanMessage.transform:FindChild("jiesuan_"..i).gameObject);
	end
	...
	this.but = ResManager:LoadAsset("gameqbsk/but","but") 
...
function this:Awake()
	...
	local footInfoPrb = ResManager:LoadAsset("gameqbsk/footinfoprb","FootInfoPrb")
	...
	--添加音效资源
	for i=1,9 do
		UISoundManager.AddAudioSource("gameqbsk/sounds","man_"..i);
		UISoundManager.AddAudioSource("gameqbsk/sounds","woman_"..i);
	end
	for i=0,12 do
		UISoundManager.AddAudioSource("gameqbsk/sounds","man_type_"..i);
		UISoundManager.AddAudioSource("gameqbsk/sounds","woman_type_"..i);
	end
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_type_13");
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_type_14");
	
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_begin");
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_fanning");
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_onekou");
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_pingkou");
	UISoundManager.AddAudioSource("gameqbsk/sounds","man_twokou");
	
	UISoundManager.AddAudioSource("gameqbsk/sounds","woman_begin");
	UISoundManager.AddAudioSource("gameqbsk/sounds","woman_fanning");
	UISoundManager.AddAudioSource("gameqbsk/sounds","woman_onekou");
	UISoundManager.AddAudioSource("gameqbsk/sounds","woman_pingkou");
	UISoundManager.AddAudioSource("gameqbsk/sounds","woman_twokou");
	
	UISoundManager.AddAudioSource("gameqbsk/sounds","fa");
	UISoundManager.AddAudioSource("gameqbsk/sounds","pass");
	UISoundManager.AddAudioSource("gameqbsk/sounds","play");
	UISoundManager.AddAudioSource("gameqbsk/sounds","zha");
...
function this:GetTbwzPlayerCtrl(tbName,tbObj)
	...
	if tbwzTemp == nil then
		...
		if not IsNil(tbObj) then
			this._tbwzPlayerCtrl[tbName] = QBSKPlayerCtrl:New(tbObj);
...
function this:ProcessLate(messageObj)
	...
	for key,value in ipairs(current_state) do
		...
		if uid==EginUser.Instance.uid then
			if thisone==uid then
				QBSKPlayer.SetLatePutOut(lastput_type,lastput);
			end
		else
	...
	coroutine.start(QBSKPlayer.PutLateUICards,mycards);
...
function this:SetAnchorPosition(_userIndex,playerIndex)
	...
	local player = nil
	if position_span == 1 or position_span == -3 then 
...
function this:ProcessDeal(messageObj)
	...
	hasCardsPlayerCount=#(this._playingPlayerList);
	notHasCardsPlayerCount=0;
	shijiCount=hasCardsPlayerCount;
	--清除未被清除的牌
	...
	local bankerPlayer=GameObject.Find(_nnPlayerName..banker);
	...
	--发牌
	--log(#(this._playingPlayerList).."========玩家个数");
	for key,player in ipairs(this._playingPlayerList) do
		if not IsNil(player) then
			local ctrl = this:GetTbwzPlayerCtrl(player.name)
			local isBanker=false;
			if player==bankerPlayer then
				isBanker=true;
			else
				isBanker=false;
			end
			if player == this.userPlayerObj then
				...
				--log("别人发牌");
				coroutine.start(ctrl.SetDeal,ctrl,true,#(cards),isBanker,nil);
			end
			coroutine.wait(0.1);
...
function this:ProcessPlay(messageObj)
	...
	if hold_num==0  then
		...
		notHasCardsPlayerCount=notHasCardsPlayerCount+1;
	else
		shijiCount=hasCardsPlayerCount-notHasCardsPlayerCount;
	end
	
	if thisone==EginUser.Instance.uid then
		this.UserPlayerCtrl:HideOrShowTime(false);
		this.tishiCount=0;
		--QBSKPlayer.isCanSelected=false;
		this.caozuoPanel:SetActive(false);
		if #(this.autoOutCardList)>0 then
			...
			--log(systemOutCard);
			--log(autoOutCard);
			if systemOutCard==autoOutCard and #(mycards)==#(QBSKPlayer.CardObjectList) then
				--log("出牌相同");
			else
				--log("出牌不同");
				QBSKPlayer.ClearPrefab();
				coroutine.start(QBSKPlayer.PutLateUICards,mycards);
				QBSKPlayer.SetLatePutOut(put_type,put_cards);
			end
		else
			QBSKPlayer.ClearPrefab();
			coroutine.start(QBSKPlayer.PutLateUICards,mycards);
		...
		QBSKPlayer.ChongZhiCards();	
		ctrl:HideOrShowTime(false);
		this.tishiArray=QBSKPlayer.SetOtherCardInfo(put_type,put_cards);--获取提示数组
	end
	...
	local playerctrl = this:GetTbwzPlayerCtrl(_nnPlayerName..nextone);
	if playerctrl.buchuState.activeSelf then
		playerctrl:HideOrShowBuChuState(false);
	end
...
function this:ProcessPass(messageObj)
	...
	if thisone==EginUser.Instance.uid then
		...
		ctrl:DestroyCards();
		--QBSKPlayer.SetOtherCardInfo(0,nil);
		QBSKPlayer.ChongZhiCards();
	end
	...
	local jianceCount=shijiCount-1;
	if passCardIndex==jianceCount then
		huiheEnd=true;
		shijiCount=hasCardsPlayerCount-notHasCardsPlayerCount;
		QBSKPlayer.SetOtherCardInfo(0,nil)
	end
...
function this:ProcessGameover(messageObj)
	...
	local winners=body["winners"];
	local maxstar=tonumber(body["maxstar"]);
	local wintype=tonumber(body["wintype"]);--0 ""  1  双扣   2  单扣   3 平扣
	
	local winNickname="";
	for key,value in ipairs(winners) do
		--log(tostring(value));
		if tostring(value)==EginUser.Instance.uid then
			if key==1 then
				winNickname=winNickname..EginUser.Instance.nickname;
				if  LengthUTF8String(winNickname) > 3 then
					winNickname =   SubUTF8String(winNickname,3).."..";
				end
			else
				winNickname=winNickname.."、"..EginUser.Instance.nickname;
			end
			...
			local ctrl = this:GetTbwzPlayerCtrl(_nnPlayerName..tostring(value));
			if key==1 then
				winNickname=winNickname..ctrl.userNickname.text;
				if  LengthUTF8String(winNickname) > 3 then
					winNickname =   SubUTF8String(winNickname,3).."..";
				end
			else
	...
	if tostring(winners[1])==EginUser.Instance.uid then
		this.UserPlayerCtrl:PlayWinSound(wintype);
	else
		local ctrl = this:GetTbwzPlayerCtrl(_nnPlayerName..tostring(winners[1]));
		ctrl:PlayWinSound(wintype);
	end
	--log(winNickname);
	if  LengthUTF8String(winNickname) > 8 then
			winNickname =   SubUTF8String(winNickname,12).."..";
	end
	...
	local jiesuanCount=1;
	for key,value in ipairs(gameinfos) do
		if tostring(value["uid"])==EginUser.Instance.uid then
			this:SetWinMessage(value,this.jiesuan[1]);
		else
			jiesuanCount=jiesuanCount+1;
			this:SetWinMessage(value,this.jiesuan[jiesuanCount]);
		end
	end
	coroutine.wait(1.5);
...
function this.ProcessUpdateIntomoney(messageStr)
	...
	if not IsNil(info) then
		FootInfo:UpdateIntomoneyString(QBSKPlayerCtrl:NumberAddWan(intoMoney));
	end
end
...
function this:OnButtonClick(target)
	...
	elseif target==this.ButtonTiShi then
		...
		QBSKPlayer.SetTiShiMessage(this.tishiArray[this.tishiCount]);
