...
public class UIPopupList : UIWidgetContainer
{
	...
	void Highlight (UILabel lbl, bool instant)
	{
		if (mHighlight != null)
		{
            if (mHighlightedLabel != null ){
                mHighlightedLabel.color = textColor;
            }
			mHighlightedLabel = lbl;
            mHighlightedLabel.color = textHighlightColor;
	...
	void OnItemHover (GameObject go, bool isOver)
	{
		if (isOver)
		{
           ...
           go.GetComponent<UILabel>().color = textColor;
		}
	}
	...
	public void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
			...
			float h = (labelHeight + hlspHeight * scaleFactor)*0.85f;
